...
if [[ ${#ARGUMENTS[@]} -ne 5 ]]; then
    ...
    echo "         --qthresh The threshold for filtering low quality reads. (Default: 0.01)";
...
if [ "${BAMFILE: -4}" == ".bam" ]; then
    ...
    snATACOverlapCounter.jar ${FORCESORTED}--expectedoverlap ${EXPECTEDOVERLAP} --bambc ${BAMBC} --bcidx ${BCIDX} --cellidx ${CELLIDX} --iscellidx ${ISCELLIDX} --mapqthresh ${MAPQTHRESH} --maxinsertsize ${MAXINSERT} --startbases ${STARTBASES} --endbases ${ENDBASES} ${BAMFILE} ${BCMAP} ${CHRLIST} ${OUTDIR}
    AMULET.py --expectedoverlap ${EXPECTEDOVERLAP} --q ${QTHRESH} --qrep ${QTHRESH} --rfilter ${REPFILTER} ${OUTDIR}/Overlaps.txt ${OUTDIR}/OverlapSummary.txt ${OUTDIR}
    ...
    FragmentFileOverlapCounter.py --maxinsertsize ${MAXINSERT} --expectedoverlap ${EXPECTEDOVERLAP} --startbases ${STARTBASES} --endbases ${ENDBASES} ${BAMFILE} ${BCMAP} ${CHRLIST} ${OUTDIR}
    AMULET.py --expectedoverlap ${EXPECTEDOVERLAP} --q ${QTHRESH} --qrep ${QTHRESH} --rfilter ${REPFILTER} ${OUTDIR}/Overlaps.txt ${OUTDIR}/OverlapSummary.txt ${OUTDIR}
else
